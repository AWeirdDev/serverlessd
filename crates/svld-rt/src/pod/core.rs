...
impl Pod {
    ...
    ///
    /// If a sleeping worker vacancy exists, that worker is reused directly — no new spawn
    /// needed because the sleeping worker is already waiting for a `StartTask` message.
    /// If the slot was explicitly removed (None), or there is no vacancy, a fresh worker
    /// is spawned instead.
    #[inline]
    #[must_use]
    pub(super) fn create_and_warmup_worker(&mut self) -> usize {
        // Prefer reusing an existing sleeping worker over spawning a brand-new one.
        if let Some(&id) = self.vacancies.iter().next() {
            self.vacancies.remove(&id);

            // Slot still has its sleeping handle — reuse it.
            if self.workers.get(id).map_or(false, |w| w.is_some()) {
                return id;
            }

            // Slot was previously removed (handle taken), spawn a fresh worker.
        ...
        // No vacancies — allocate a new slot and spawn.
        let handle = WorkerHandle::start(self);
