...
impl JsResponse {
    ...
    pub fn get_new_fn<'s>(scope: &mut v8::PinScope<'s, '_>) -> Option<Local<'s, Value>> {
    ...
    /// `response.text()` — resolves with the body as a UTF-8 string.
    fn js_instance_text(
        scope: &mut v8::PinScope,
        args: v8::FunctionCallbackArguments,
        mut rv: v8::ReturnValue,
            ...
            let parsed = {
                ...
                match v8::json::parse(tc, text_str) {
                    ...
                    None => {
                        let exc = tc
                            .exception()
                            .unwrap_or_else(|| v8::String::new(tc, "JSON parse error").unwrap().cast());
...
impl<'s> ResponseBuilder<'s> {
    ...
    #[must_use]
    pub fn headers(self, scope: &v8::PinScope<'s, '_>, headers: Local<'s, v8::Object>) -> Option<Self> {
